...
export const searchAdminCasesQuery = `
    ...
    FROM
        ...
        SELECT
            ...
            END AS LSTUPDT,
            CSD.PRIORITY,
        (select count(*) from maillogs where MAILFOR='Case' and MAILFORID=CSD.CIDSYS AND ISDEL='N') MAILCOUNT,
        (select count(*) from t_tasks where TASKOF='Case' and TASKOFID=CSD.CIDSYS AND ISDEL='N') TASKCOUNT,
        (select count(*) from TIMAGE where CENTITY='Case' and CENTITYID=CSD.CIDSYS AND ISDEL='N') LEADNOTECOUNT,
        (SELECT  NOTE FROM TIMAGE WHERE CLNT = CSD.CLNT AND LANG = CSD.LANG AND CENTITYID = CSD.CIDSYS AND ISDEL = 'N' ORDER BY CDATE,  CTIME  DESC LIMIT 1 ) ADDCOMMETS
    FROM
        CLIENTDETAILS CLD
    LEFT JOIN CASEDETAILS CSD ON
        CLD.CLNTID = CSD.CLIENTID AND CLD.ISDEL = 'N' AND CSD.ISDEL = 'N' AND CSD.ISCLOSED = 'N'
...
export const searchUserLeadsQuery_20190821 = `
    SELECT
        ...
        (select count(*) from maillogs where MAILFOR='Lead' and MAILFORID=TINQ.CID) MAILCOUNT,
        (select count(*) from t_tasks where TASKOF='Lead' and TASKOFID=TINQ.CID) TASKCOUNT,
        ( SELECT  NOTE FROM TIMAGE  WHERE  CLNT = TINQ.CLNT AND LANG = TINQ.LANG AND CENTITYID = TINQ.CID AND ISDEL = 'N'  ORDER BY
                CDATE, CTIME DESC LIMIT 1 ) ADDCOMMETS, 
        CASE WHEN TINQ.UDATE IS NULL THEN TINQ.CDATE 
            ELSE TINQ.UDATE
        END AS LSTUPDT
    FROM
